...
describe("escrow", () => {
  ...
  test("Cancel offer by Alice closes offer and vault accounts", async () => {
  const getTokenBalance = getTokenBalanceOn(connection);
  ...
  const balanceBefore = await getTokenBalance(aliceUsdcAccount);
  ...
  const balanceAfter = await getTokenBalance(aliceUsdcAccount);
  expect(balanceAfter.gt(balanceBefore)).toBe(true); // vault tokens returned

  // Expect fetching closed accounts to fail
  await expect(program.account.offer.fetch(offerAddress)).rejects.toThrow();
  await expect(connection.getAccountInfo(vaultAddress)).resolves.toBeNull();
});
});