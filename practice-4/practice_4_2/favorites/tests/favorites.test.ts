...
describe("favorites", () => {
  ...
  it("Allows a delegate to update favorites", async () => {
    const user = web3.Keypair.generate();
    const delegate = web3.Keypair.generate();
    const program = anchor.workspace.Favorites as Program<Favorites>;
  
    await airdropIfRequired(anchor.getProvider().connection, user.publicKey, 1e9, 2e9);
    await airdropIfRequired(anchor.getProvider().connection, delegate.publicKey, 1e9, 2e9);
  
    const initialNumber = new anchor.BN(99);
    const initialColor = "orange";
    ...
    const updated = await program.account.favorites.fetch(favoritesPda);
    expect(updated.color).toBe(newColor);
    expect(updated.number.toNumber()).toBe(initialNumber.toNumber());
  });
});
